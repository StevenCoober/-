<template>
  ...
  >
    <el-table-column v-if="canselect" type="selection" width="55">
            ...
            <el-table-column label="课程主题id" prop="id">
            </el-table-column>
            <el-table-column label="详细名称" prop="name">
            </el-table-column>
...
<!--             <el-table-column label="教练id" prop="coach_id">
            ...
            <el-table-column label="教练名字" prop="coach_name">
            </el-table-column>
<!--             <el-table-column label="课程主题类型id" prop="topic_type_id">
            </el-table-column> -->
            <el-table-column label="课程主题类型名称" prop="topic_type_name">
            </el-table-column>
            <!-- <el-table-column style="white-space: pre-line;" label="简介" prop="desc">
            </el-table-column> -->
            ...
            <el-table-column label="节数" prop="class_num">
            </el-table-column>
            <el-table-column label="金额" prop="class_price">
            </el-table-column>
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Course_topic", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Course_topic", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
      ...
      if (res["data"] != null) {
        ...
        for (let i = 0; i < resData["data"].length; i++) {
          ...
          res = await this.request({
            url: "/admin/course_topic_type/info",
            data: {
              id: topic_type_id
            },
            method: "post"
          });
          resData["data"][i].topic_type_name  = res.data.type_name;
    ...
    async onSelectItem(row) {
      if (row.id != null) {
        this.pageSize = 1;
        await this.FetchInfosList("course_topic_id", row.id);
      }
    },
    /////////////////////////////////////////////////////
    // 查询数据项
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Course_topic() {
      await this.FetchIndexList();
    },
  }
};
