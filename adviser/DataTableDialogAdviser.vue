<template>
  ...
  >
    ...
    <el-col :span="24" class="toolbar" style="text-align:left">
      ...
      <!--新增按钮-->
      <el-button @click="OnClickNewItem" icon="el-icon-plus"  type="success">新增顾问</el-button>
    </el-col>
...
export default {
  ...
  methods: {
    OnClickQuery() {
      xxevent.emit("SendQueryTableItem_Adviser", this.queryItemKey, this.queryItemName); // 查询数据项
    },
    
    OnClickNewItem() {
      xxevent.emit("EditDialogShow_Adviser", true); // 新增或编辑界面显示
      this.$nextTick(() => xxevent.emit("NewObj_Adviser")); // 编辑数据
    },

    /////////////////////////////////////////////////////
    
    // 显示查询数据项窗体
    onQueryTableDialogShow_Adviser(isShow) {
      this.dialogVisible = isShow;
    },
    // 查询数据项
    async onSendQueryTableItem_Adviser(queryItemKey, queryItemName) {
