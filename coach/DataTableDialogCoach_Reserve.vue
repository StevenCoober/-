<template>
  ...
  >
    ...
    <el-col :span="24" class="toolbar" style="text-align:left">
      ...
      <!--新增按钮-->
      <el-button @click="OnClickNewItem" icon="el-icon-plus"  type="success">新增教练预约订单</el-button>
    </el-col>
...
export default {
  name: "DataTableDialogCoach_Reserve",
  components: {
    ImageCropper,
    PanThumb,
    Tinymce
  ...
  methods: {
    OnClickQuery() {
      xxevent.emit("SendQueryTableItem_Coach_Reserve", this.queryItemKey, this.queryItemName); // 查询数据项
    },
    
    OnClickNewItem() {
      xxevent.emit("EditDialogShow_Coach_Reserve", true); // 新增或编辑界面显示
      this.$nextTick(() => xxevent.emit("NewObj_Coach_Reserve")); // 编辑数据
    },
