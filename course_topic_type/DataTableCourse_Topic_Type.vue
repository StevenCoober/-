<template>
  ...
  >
    <el-table-column v-if="canselect" type="selection" width="55">
            ...
            <el-table-column label="课程主题类型id" prop="id">
            </el-table-column>
            <el-table-column label="类型名称, 比如瘦身课" prop="type_name">
            </el-table-column>
...
export default {
  name: "DataTableCourse_Topic_Type",
  components: {
    ImageCropper,
    PanThumb,
    Tinymce,
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Course_Topic_Type", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Course_Topic_Type", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Course_Topic_Type() {
      await this.FetchIndexList();
    },
  }
};
