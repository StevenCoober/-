<template>
  ...
  >
    <el-table-column v-if="canselect" type="selection" width="55">
            ...
            <el-table-column label="访客id" prop="id">
            </el-table-column>
            <el-table-column label="会员卡id" prop="member_card_id">
            </el-table-column>
<!--             <el-table-column label="会员" style="width:100%;"  prop="member_name">
            </el-table-column> -->
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Visitor", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Visitor", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    async FetchIndexList() {
      ...
      if (res["data"] != null) {
        ...
        for (let i = 0; i < resData["data"].length; i++) {
          ...
          //   data: {
          //     id: member_card_id, // 会员用户id
          //   },
          //   method: "post"
          // });
          // const member_name = res.data.name;
    ...
    async onSelectItem(row) {
      if (row.id != null) {
        this.pageSize = 1;
        await this.FetchInfosList("visitor_id", row.id);
      }
    },
    /////////////////////////////////////////////////////
    // 查询数据项
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Visitor() {
      await this.FetchIndexList();
    },
  }
};
