...
<!--             <el-table-column label="私教课id(教练特长id)" prop="course_id">
            ...
            <el-table-column label="私教课" prop="course_name">
            </el-table-column>
            ...
            <el-table-column label="私教课购买类型" prop="course_type">
            </el-table-column>
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Private_Course", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Private_Course", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    async FetchIndexList() {
	  ...
	  const course_type = data.course_type; // 私教课购买类型
          ...
          if (res.data.name != null) {
            ...
            let skillOptions = res.data.skill.split(',');
            resData["data"][i].course_name = skillOptions[course_id];
          }
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Private_Course() {
      await this.FetchIndexList();
    },
  }
};
