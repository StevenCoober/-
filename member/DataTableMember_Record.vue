...
<!--             <el-table-column label="微信用户id" prop="user_id">
            ...
            <el-table-column label="请求状态" >
              <template slot-scope="props">
                <span>{{['未处理', '已处理'][props.row.state]}}</span>
              </template>
            </el-table-column>
            ...
            <el-table-column label="顾问" prop="adviser_name">
            </el-table-column>
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Member_Record", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Member_Record", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    async OnClickDeleteItem(index, row) {
      ...
      let res = await this.request({
        url: "/admin/member_record/delete",
        data: { id: row.id }, // 会员用户录入 id
        method: "post"
      });
      if (res.errno == 0) {
    ...
    async FetchIndexList() {
      ...
      if (res["data"] != null) {
        ...
        for (let i = 0; i < resData["data"].length; i++) {
          ...
          const adviser_id = data.adviser_id;
          ...
          resData["data"][i].adviser_name  = res.data.name;
        }
    ...
    /////////////////////////////////////////////////////
    // 查询数据项
    async onSendQueryTableItem_Member_Record(queryItemKey, queryItemName) {
      this.currentPage = 1;
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Member_Record() {
      await this.FetchIndexList();
    },
  }
};
