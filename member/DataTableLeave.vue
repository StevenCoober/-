  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Leave", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Leave", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Leave() {
      await this.FetchIndexList();
    },
  }
};
