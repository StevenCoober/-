<template>
  ...
  >
    <el-table-column v-if="canselect" type="selection" width="55">
            ...
            <el-table-column label="会员用户id" prop="id">
            </el-table-column>
            ...
            <el-table-column label="身份证" prop="identity_card">
            </el-table-column>
            <el-table-column label="生活照" prop="life_photo">
              <template slot-scope="props">
                <image-cropper-more :single="true" v-model="props.row.life_photo" :disabled="true"></image-cropper-more>
              </template>
            </el-table-column>
            <el-table-column label="顾问id" prop="adviser_id">
            </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="props">
          ...
          <el-button
            ...
            icon="el-icon-share"
            style="margin-left: 10px;"
            @click="OnClickUnSetMember(props.$index, props.row)"
          >取消绑定</el-button>
          <el-button
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Member", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Member", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    async FetchIndexList() {
      ...
      if (res["data"] != null) {
        ...
        for (let i = 0; i < resData["data"].length; i++) {
          ...
          const identity_card = data.identity_card; // 身份证
          const life_photo = data.life_photo; // 生活照
          const adviser_id = data.adviser_id; // 顾问id
        }
    ...
    async onSelectItem(row) {
      if (row.id != null) {
        this.pageSize = 1;
        await this.FetchInfosList("member_id", row.id);
      }
    },
    /////////////////////////////////////////////////////
    // 查询数据项
    async onSendQueryTableItem_Member(queryItemKey, queryItemName, user_id) {
      this.currentPage = 1;
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Member() {
      await this.FetchIndexList();
    },
    ...
    async OnClickSetMember(index, row) {
      ...
      if (res.errno == 0) {
        ...
        xxevent.emit("QueryTableDialogShow_Member", false); // 新增或编辑界面显示
    ...
    async OnClickUnSetMember(index, row) {
      let data = {
        id: row.id,
        user_id: 0
      }
