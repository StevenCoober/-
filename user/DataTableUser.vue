<template>
  ...
  >
    <el-table-column v-if="canselect" type="selection" width="55">
            ...
            <el-table-column label="人员id" prop="id">
            </el-table-column>
            ...
            <el-table-column label="注册ip" prop="register_ip">
            </el-table-column>
            ...
            <el-table-column label="微信openid" prop="weixin_openid">
            </el-table-column>
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_User", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_User", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    async onSelectItem(row) {
      if (row.id != null) {
        this.pageSize = 1;
        await this.FetchInfosList("user_id", row.id);
      }
    },
    /////////////////////////////////////////////////////
    // 查询数据项
