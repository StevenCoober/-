    ...
    <el-table-column label="操作">
        <template slot-scope="props">
         ...
         <el-button
            ...
            icon="el-icon-share"
            style="margin-left: 10px;"
            @click="OnClickUnSetAdviser(props.$index, props.row)"
          >取消绑定</el-button> 
          <el-button
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Adviser", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Adviser", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    /////////////////////////////////////////////////////
    // 查询数据项
    async onSendQueryTableItem_Adviser(queryItemKey, queryItemName, user_id) {
      this.currentPage = 1;
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Adviser() {
      await this.FetchIndexList();
    },
    ...
    async OnClickSetAdviser(index, row) {
      ...
      if (res.errno == 0) {
        ...
        xxevent.emit("QueryTableDialogShow_Adviser", false); // 新增或编辑界面显示
        xxevent.emit("SendUpdateTableItem_User"); // 更新查询数据项
        xxevent.emit("SendUpdateTableItem_Adviser"); // 更新查询数据项
        xxevent.emit("SendUpdateItem_Adviser", res.data); // 更新单个数据项
      }
    },

    async OnClickUnSetAdviser(index, row) {
      let data = {
        id: row.id,
        user_id: 0
      }
