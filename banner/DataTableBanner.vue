<template>
  ...
  >
    <el-table-column v-if="canselect" type="selection" width="55">
            ...
            <el-table-column label="广告位id" prop="id">
            </el-table-column>
            <el-table-column label="图片地址" prop="pic_url">
                    ...
                    <el-card class="box-card">
                      <div class="component-item">
                        <pan-thumb
                          width="100px"
                          height="100px"
                          :image="props.row.pic_url"
                        ></pan-thumb>
                      </div>
                    </el-card>
              </template>
            </el-table-column>
            <el-table-column label="广告图片类型ad, env" prop="type">
            </el-table-column>
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Banner", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Banner", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    async onSelectItem(row) {
      if (row.id != null) {
        this.pageSize = 1;
        await this.FetchInfosList("banner_id", row.id);
      }
    },
    /////////////////////////////////////////////////////
    // 查询数据项
    async onSendQueryTableItem_Banner(queryItemKey, queryItemName) {
      this.currentPage = 1;
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Banner() {
      await this.FetchIndexList();
    },
  }
};
