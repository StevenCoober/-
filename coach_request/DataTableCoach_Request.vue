<template>
  ...
  >
    <el-table-column v-if="canselect" type="selection" width="55">
            ...
            <el-table-column label="教练申请请求id" prop="id">
            </el-table-column>
            ...
            <el-table-column label="带课量" prop="num">
            </el-table-column>
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Coach_Request", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Coach_Request", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    async onSelectItem(row) {
      if (row.id != null) {
        this.pageSize = 1;
        await this.FetchInfosList("coach_request_id", row.id);
      }
    },
    /////////////////////////////////////////////////////
    // 查询数据项
    async onSendQueryTableItem_Coach_Request(queryItemKey, queryItemName) {
      this.currentPage = 1;
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Coach_Request() {
      await this.FetchIndexList();
    },
  }
};
