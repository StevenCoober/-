  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Member_Request", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Member_Request", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    async onSelectItem(row) {
      if (row.id != null) {
        this.pageSize = 1;
        await this.FetchInfosList("member_request_id", row.id);
      }
    },
    /////////////////////////////////////////////////////
    // 查询数据项
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Member_Request() {
      await this.FetchIndexList();
    },
  }
};
