<template>
  ...
  >
    ...
    <el-col :span="24" class="toolbar" style="text-align:left">
      ...
      <!--新增按钮-->
      <el-button @click="OnClickNewItem" icon="el-icon-plus"  type="success">新增会员用户录入</el-button>
    </el-col>
...
export default {
  name: "DataTableDialogMember_Record",
  components: {
    ImageCropper,
    PanThumb,
    Tinymce
  ...
  methods: {
    OnClickQuery() {
      xxevent.emit("SendQueryTableItem_Member_Record", this.queryItemKey, this.queryItemName); // 查询数据项
    },
    
    OnClickNewItem() {
      xxevent.emit("EditDialogShow_Member_Record", true); // 新增或编辑界面显示
      this.$nextTick(() => xxevent.emit("NewObj_Member_Record")); // 编辑数据
    },
