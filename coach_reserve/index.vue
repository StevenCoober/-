  ...
  <div class="app-container">
    <div class="filter-container">
      <el-form :inline="true" class="demo-form-inline">
        ...
        <!--新增按钮-->
        <el-button @click="OnClickAddItem" icon="el-icon-plus" type="success">新增教练预约订单</el-button>
        ...
        <template>
          ...
          >
            ...
            <el-table-column type="expand">
              <template slot-scope="props">
                <el-form label-position="left" inline class="demo-table-expand">
                  <el-form-item label="教练预约订单id" style="width:100%;">
                    <span>{{props.row.id}}</span>
                  </el-form-item>
                  <el-form-item label="预定的会员用户id" style="width:100%;">
                    <span>{{props.row.user_id}}</span>
                  </el-form-item>
                  <el-form-item label="教练id" style="width:100%;">
                    <span>{{props.row.coach_id}}</span>
                  </el-form-item>
                  <el-form-item label="预约课程id" style="width:100%;">
                    <span>{{props.row.skill_id}}</span>
                  </el-form-item>
                  <el-form-item label="订单状态//0未开始1进行中2已完成" style="width:100%;">
                    <span>{{props.row.order_state}}</span>
                  </el-form-item>
                  <el-form-item label="预约结果状态" style="width:100%;">
                    <span>{{props.row.orde_result_state}}</span>
                  </el-form-item>
                  <el-form-item label="预约开始时间 " style="width:100%;">
                    <el-icon name="time"></el-icon>
                    <span>{{props.row.reverse_btime}}</span>
                  </el-form-item>
                  <el-form-item label="预约结束时间 " style="width:100%;">
                    <el-icon name="time"></el-icon>
                    <span>{{props.row.reverse_etime}}</span>
                  </el-form-item>
            ...
            <!--++隐藏的项++-->
            <!--++显示的项++-->
            <el-table-column label="教练预约订单id" prop="id"></el-table-column>
            <el-table-column label="预定的会员用户id" prop="user_id"></el-table-column>
            <el-table-column label="教练id" prop="coach_id"></el-table-column>
            <el-table-column label="预约课程id" prop="skill_id"></el-table-column>
            <el-table-column label="订单状态//0未开始1进行中2已完成" prop="order_state"></el-table-column>
            <el-table-column label="预约结果状态" prop="orde_result_state"></el-table-column>
            <el-table-column label="预约开始时间 " prop="reverse_btime">
              <template slot-scope="props">
                    <el-icon name="time"></el-icon>
                    <span>{{props.row.reverse_btime}}</span>
              </template>
            </el-table-column>
            <el-table-column label="预约结束时间 " prop="reverse_etime">
              <template slot-scope="props">
                    <el-icon name="time"></el-icon>
                    <span>{{props.row.reverse_etime}}</span>
              </template>
      ...
      <!--Edit It Begin -->
      <el-dialog title="教练预约订单" :visible.sync="dialogFormVisible">
...
>
      ...
      <!--Edit It End -->
      <el-dialog title="教练预约订单" :visible.sync="tinymceDialogFormVisible" width="100%">
        <template>
  ...
  data() {
    return {
      ...
      importanceOptions: { 
        ...
        orde_result_state: "预约结果状态", 
        reverse_btime: "预约开始时间 ", 
        reverse_etime: "预约结束时间 ", 
      }, // 查询类型索引数据
      tableData: [], // 查询的数据结果
  ...
  methods: {
    ...
    async OnClickDeleteItem(index, row) {
      ...
      let res = await this.request({
        url: "/admin/coach_reserve/delete",
        data: { id: row.id}, // 教练预约订单id
        method: "post"
      });
      if (res.errno == 0) {
    ...
    async FetchList() {
      ...
      //   url: "/admin/coach_reserve/all",
      //   method: "post"
      // });
      // if (res_all["data"] != null) {
      //   for (let i = 0; i < res_all["data"].length; i++) {
      //     const data = res_all["data"][i]; // 教练预约订单
      ...
      let res = await this.request({
        url: "/admin/coach_reserve/allIndex",
        data: data,
        method: "post"
      }); 
      ...
      if (res["data"] != null) {
        ...
        for (let i = 0; i < resData["data"].length; i++) {
          ...
          const id = data.id; // 教练预约订单id
          const coach_id = data.coach_id; // 教练id
          const skill_id = data.skill_id; // 课程id
          const order_state = data.order_state; // 订单状态//0未开始1进行中2已完成
          const orde_result_state = data.orde_result_state; // 预约结果状态
          const reverse_btime = data.reverse_btime; // 预约开始时间
          const reverse_etime = data.reverse_etime; // 预约结束时间
          const user_id = data.coach_reserve_use_id; //会员用户id
          const coach_name = data.coach_name; // 教练名字
          const skill= data.skill; // 教练技能

          resData["data"][i].skill = skill.split(',');
        }
