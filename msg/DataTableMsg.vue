<template>
  ...
  >
    <el-table-column v-if="canselect" type="selection" width="55">
            ...
            <el-table-column style="white-space: pre-line;" label="消息内容" prop="content">
            </el-table-column>
            ...
            <el-table-column label="微信用户是否显示" prop="is_show">
            </el-table-column>
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Msg", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Msg", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    async onSelectItem(row) {
      if (row.id != null) {
        this.pageSize = 1;
        await this.FetchInfosList("msg_id", row.id);
      }
    },
    /////////////////////////////////////////////////////
    // 查询数据项
    async onSendQueryTableItem_Msg(queryItemKey, queryItemName) {
      this.currentPage = 1;
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Msg() {
      await this.FetchIndexList();
    },
  }
};
