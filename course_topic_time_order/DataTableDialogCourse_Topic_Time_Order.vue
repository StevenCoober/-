<template>
  ...
  >
    ...
    <el-col :span="24" class="toolbar" style="text-align:left">
      ...
      <!--新增按钮-->
      <el-button @click="OnClickNewItem" icon="el-icon-plus"  type="success">新增课程主题用户预定情况管理</el-button>
    </el-col>
...
export default {
  name: "DataTableDialogCourse_Topic_Time_Order",
  components: {
    ImageCropper,
    PanThumb,
    Tinymce
  ...
  methods: {
    OnClickQuery() {
      xxevent.emit("SendQueryTableItem_Course_Topic_Time_Order", this.queryItemKey, this.queryItemName); // 查询数据项
    },
    
    OnClickNewItem() {
      xxevent.emit("EditDialogShow_Course_Topic_Time_Order", true); // 新增或编辑界面显示
      this.$nextTick(() => xxevent.emit("NewObj_Course_Topic_Time_Order")); // 编辑数据
    },
