<template>
  ...
  >
    <el-table-column v-if="canselect" type="selection" width="55">
            ...
            <el-table-column label="配置id" prop="id">
            </el-table-column>
            <el-table-column label="配置说明" prop="desc">
            </el-table-column>
            <el-table-column label="项关键字" prop="key">
            </el-table-column>
            <el-table-column style="white-space: pre-line;" label="项关键词" prop="value">
            </el-table-column>
  ...
  methods: {
    ...
    */
    OnSelect(rows) {
      ...
      xxevent.emit("SelectTableItem_Config", row || {}); // 选中查询结果某项
    },
    ...
    OnClickEditItem(index, row) {
      ...
      this.$nextTick(() => xxevent.emit("EditObj_Config", deepClone(row))); // 编辑数据
      this.$emit("edit", row || {});
    },
    ...
    async onSelectItem(row) {
      if (row.id != null) {
        this.pageSize = 1;
        await this.FetchInfosList("config_id", row.id);
      }
    },
    /////////////////////////////////////////////////////
    // 查询数据项
    async onSendQueryTableItem_Config(queryItemKey, queryItemName) {
      this.currentPage = 1;
    ...
    // 更新查询数据项
    async onSendUpdateTableItem_Config() {
      await this.FetchIndexList();
    },
  }
};
